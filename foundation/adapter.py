...
def format_question_bank(doc: FoundationDocument) -> str:
    ...
    if subjective_count:
        parts.append(
            f"  Excluded Questions: {subjective_count} (True/False + subjective — see separate download)"
        )
    parts += [_HEADER_EQ, ""]
...
def is_subjective_q(q: FoundationQuestion) -> bool:
    """Return True for questions that go into the separate download (subjective + true/false)."""
...
def format_subjective_bank(doc: FoundationDocument) -> str:
    """Format subjective + excluded (True/False) questions into a separate file."""
    ...
    parts: list[str] = [
        ...
        title_line,
        f"  Foundation {doc.subject} | Class {doc.class_level}",
        f"  Total Excluded Questions: {len(subj_qs)}",
        "  NOTE: True/False questions and subjective questions (VSA/SA/LA/Descriptive) are excluded from the main MCQ bank.",
        _HEADER_EQ,
        "",
    ]
